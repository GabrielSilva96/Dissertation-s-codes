...
// Description: Continuous PWM increments until the maximum value, followed by decrements 
...
long k=1.0, p=1.0, on_time = 100000, off_time = 180000;
...
void loop() {
  ...
  { 
    pwm_value =  25*(j-2); 
    if (curr_t>= on_time*(11) + off_time*p)
    {
      j -= 1;
      p +=1;
    }
  ...
  if (curr_t>= 11*on_time + 10*off_time)
  {
    Serial.end();
    Serial.println("END"); 
  }
