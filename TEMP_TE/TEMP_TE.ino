...
// Description: Temperature acquisition for the TE model
...
long k=1.0, p=1.0, on_time = 5000, off_time = 150000, start_t = 5000;
...
void loop() {
  ...
  if (curr_t<= on_time*(k) + off_time*(k-1.0) + start_t)
  {
    pwm_value =  0;
    if (curr_t>= start_t){
      pwm_value =  225;
  ...
  { 
    pwm_value = 0;
    if (curr_t>= on_time*k + off_time*k + start_t)
    {
      k += 1.0;  
      j += 1;
    }
  ...
  TEMP_ACQ();
   ...
   if (curr_t>= on_time*1 + off_time*1 + start_t)
  {
